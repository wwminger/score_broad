<template>
  <div class="price_gf" v-if="recData">
    <div class="rec_wrapper">
      ...
      <div v-for="rec in recData" class="rec_box" :style="{ width: recW + '%', height: recMaxH + 'px' }" :key="rec.id">
        <div v-if="rec.show" class="rec_item_wrapper">
          <p class="num">{{ rec.value }}</p>
          <div class="rec_item" :style="{ height: showRecH ? rec.height : '0' }">
            <div class="star_icon"></div>
          </div>
          <div class="btn">
            <!-- <div v-on:click="rec.value=rec.value+1">
              number{{rec.name}}
            </div> -->
            <!-- <input type="button" v-on:click="rec.value=rec.value+1" value={{rec.name}} :style="{ width: recW * 2 + '%' } " :key="rec.id"> -->
            <el-button  :key="rec.id" @click="cadd(rec.id)">
              {{ rec.name }}
            </el-button>
          </div>
    ...
    <div class="title_box">
      <span class="title" v-for="item in recData" :style="item.show ? { width: recW * 2 + '%' } : { display: 'none' }"
...
export default {
  ...
  methods: {
    ...
    cadd(key) {
       ...
       */
      console.log(key);
      console.log(this.gfData.at(key - 1).value);
      this.gfData.at(key - 1).value = parseInt(this.gfData.at(key - 1).value, 10) + 1;
      this.structRecData();
    },
  },
}
